...
ng::json Settings::jsonFile;
unsigned Settings::numberOfFiles = 0;
...
void Settings::load()
{
	...
	if (in.fail()) { // if failed to load
		...
		jsonFile[renderWindowKey][widthKey] = 1024;
		jsonFile[renderWindowKey][heightKey] = 576;
		jsonFile[renderWindowKey][typeKey] = static_cast<int>(ng::WINDOW_TYPE::WINDOW_UNRESIZEABLE);
		jsonFile[consoleWindowKey][widthKey] = 800;
		jsonFile[consoleWindowKey][heightKey] = 480;
		jsonFile[consoleWindowKey][isDisabled] = false;
		jsonFile[appVolumeKey] = 0.5F;
		jsonFile[introLocKey] = std::string(INPUT_LOC) + std::string("intro.mp3");
		jsonFile[outroLocKey] = std::string(INPUT_LOC) + std::string("outro.mp3");
		jsonFile[autoInOutroKey] = false;
		jsonFile[appVolumeKey] = 0.5F;
		jsonFile[dimActiveKey] = false;
		jsonFile[dimVolumeKey] = 0.2F;
		jsonFile[dimThresholdKey] = 0.2F;
		jsonFile[dimDurationKey] = 1.5F;
		...
		for (int i = 0; i < static_cast<int>(numberOfFiles); i++) {
			jsonFile[std::to_string(i)][startKey] = starts[i];
			jsonFile[std::to_string(i)][endKey] = ends[i];
			jsonFile[std::to_string(i)][fileKey] = std::to_string(i) + ".szunet.mp3";
		}
		...
		while (jsonFile[address][startKey] != nullptr) {
...
void Settings::save()
{
	...
	if (out.fail())
	{
		ng::ConfirmDialog errorDialog
		{ng::ConfirmDialog::DIALOG_TYPE::DIALOG_CLOSE_ONLY};
		
		errorDialog.setSize({ 775.0F, 250.0F });
		errorDialog.setPosition({0, 0});
		errorDialog.setShapeColor({ 60, 60, 60 });
		errorDialog.setFont(NG_FONT("arial_font.ttf"));
		errorDialog.setAllTextColor({ 243, 96, 0 });
		errorDialog.setCharacterSize(30);
		errorDialog.setButtonTexture(NG_TEXTURE("button.png"));
		errorDialog.setTextYOffset(-30);
		errorDialog.setString(
			"Unexpected error while writing settings.json file!\n"
			"Please contact Szoke Andras-Lorand if error persists!"
		);

		errorDialog.setIsActive(true);
		errorDialog.drawInWindow("Error!");
		...
		jsonFile["!comment1"] = "You can add or remove breaks here and change system settings manually!";
		jsonFile["!comment2"] = "However proceed with caution! Things might break roun' here.";
		jsonFile["!comment3"] = "If things do break delete this file and start program to reset defaults";
		jsonFile["!comment4"] = "For more information contact me, Sz.Lori on social media :D";;

		out << jsonFile.dump(4); // print out in an organised way
		out.close();
...
void Settings::handleEvents(const sf::Event& event)
{
	...
	if (settingsSwitcher_.isActive() && !confirmDialog_.isActive())
	{
		...
		jsonFile[appVolumeKey] = maxVolumeSlider_.getLevel();
		jsonFile[dimVolumeKey] = dimVolumeSlider_.getLevel();
		jsonFile[dimThresholdKey] = dimThresholdSlider_.getLevel();
	}
...
void Settings::applyNewSettings(const CHANGE changeType)
{
	sf::Vector2u winRes;
	sf::Vector2u conRes;
	...
	else if (changeType == CHANGE::CHANGE_PERMANENT)
	{
		...
		jsonFile[renderWindowKey][widthKey] = winRes.x;
		jsonFile[renderWindowKey][heightKey] = winRes.y;
		jsonFile[renderWindowKey][typeKey] = static_cast<int>(ng::Main::windowType);
		jsonFile[consoleWindowKey][widthKey] = conRes.x;
		jsonFile[consoleWindowKey][heightKey] = conRes.y;
